...
with open(concept_file_path, 'r') as file:
    content = file.read()
    concept_list = ast.literal_eval(content)[9:17]

with open(image_file_path, 'r') as file:
    content = file.read()
    image_list = ast.literal_eval(content)[9:17]
...
for each_concept, each_image in zip(concept_list, image_list):
    ...
    for each_index, each_prompt in enumerate(prompt_list):
        ...
        for i in range(20):
            ...
            image_final[0].save(f"./trytry/{each_image}_prompt{each_index}_sample{i}.png")
